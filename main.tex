...
\begin{frame}[fragile]{\textbf{Q. What is Supervised Learning?}}
  \begin{wideitemize}
    ...
    \item \textcolor{blue}{You could think of these labels as the name of the
...
\begin{frame}[fragile]{\textbf{Q. Can you give an example of Supervised Learning?}}
  ...
  \begin{wideitemize}
    ...
    goal is to construct the optimal separating \textit{hyperplane} between
    pieces of data given input data; e.g., a plane separating A from B.
    \begin{wideitemize}
      \item[-] A and B sit in some high dimensional space, and the idea is to
...
\begin{frame}[fragile]{\textbf{Q. What more can you say about SVM?}}
  ...
  \begin{wideitemize}
    ...
    the kernel trick\footnote{adds one more dimension and calls it $z$-axis -- governed by the
    constraint $z = x^2 + y^2$ and $z$ is the square of distance of the point from origin.} to convert
    these data to linearly separable data in a higher dimension. 
    ...
    \begin{wideitemize}
     \item $C$ is regularization parameter that controls the trade-off between smooth decision boundary
     and classifying training points correctly. A large $C$ means you'll get more training
     points correctly.
     \item \textit{Gamma}. It defines how far the influence of a single training example reaches.
...
\begin{frame}[fragile]{\textbf{Q. Can you give an example of Unsupervised Learning?}}
  \begin{wideitemize}
    ...
    \item In \textbf{PCA}, you diagonalize $C$ and say $C = \sum_{k} P_k
     ...
     of matrices involving involving eigenvalues and eigenvectors}, $P_k$ is piece of the data with size $k$,
...
\begin{transitionsubframe}
  \begin{center}
    \Huge Our evaluation framework and cross-validation
  \end{center}
\end{transitionsubframe}
...
\subsection{Epoch vs Batch size vs Iteration}
\begin{transitionsubframe}
  \begin{center}
...
\begin{frame}[fragile]{\textbf{Q. What is Regression?}}
  ...
  the relationship among variables, often to make a prediction about
  some outcome.
  \end{wideitemize}
\end{frame}
...
\begin{frame}[fragile]{\textbf{Q. What is linear regression?}}
  ...
  you have continuous target variable (output).
...
\begin{frame}[fragile]{\textbf{Q. What is logistic regression?}}
  \begin{wideitemize}
  \item \textbf{Logistic regression} is another type of regression where the
  target variable is binary (zero or one, or true or false). It takes the form:
  \begin{wideitemize}
    \item $\frac{1}{1 + e^{-(mx + b)}}$
  \end{wideitemize}
  \end{wideitemize}
\end{frame}
...
\begin{frame}[fragile]{\textbf{Q. Why won't Linear Regression work for binary target variables?}}
  \begin{wideitemize}
  \item In other words, why do we need two regression algorithms?
  i.e., linear and logistic regression.
  \item Because \textbf{Linear regression} for binary target variable will have
  a hard time try to come up with a best fit line that makes any sense. It will
  try to fit a line that fits all of the data and it will end up predicting
  negative values and values over one, which is impossible.
  \item \textbf{Logistic regression} is built off of a logistic or sigmoid curve
  which looks an S shape. This will always be between zero and one, which makes it
  a better fit for a binary classification problem.
  \end{wideitemize}
\end{frame}
