...
\author{
% \author[HAS \& DS]{
\parbox[t]{1.5in}{Huascar Sanchez \\{\scriptsize\texttt{huascar.sanchez@sri.com}}} %\hspace{.3in}
% \and
% \parbox[t]{1.5in}{Second Author \\\small\texttt{second.author@sri.com}}
}
\institute{CSL @ SRI International}
...
\begin{frame}[fragile]{Open-source code security in one slide}
  ...
  \begin{wideitemize}
    ...
    \end{wideitemize}
    % Nothing will ever be completely breach-free.
    % But, we should not measure success on this. Right?
    ...
    % \item \textbf{So what can we do?} we can provide guidance over which
    % of portions of new code can be relied upon, and which others require closer
    % scrutiny --- without slowing down productivity.
    % We must fundamentally change the lens around the security lifecycle and address the
    % fact that before software grows up  it is born as code --- in version control --
    % thru interactions.
  \end{wideitemize}
\end{frame}

\begin{frame}[fragile]{Solution landscape is fragmented}
  \setlength{\leftmarginii}{0.2cm}
  \begin{wideitemize}
    ...
    % The fact is that ...
    \begin{wideitemize}
      % No wonder we see Equifax-sized meltdowns in the world pretty regularly now.
      \item \shade{Red!20}{We're} ignoring the root cause of the issue and just stacking more
      ...
      % The fact is that ...
      % Once in the wild, it's laborious, expensive and challenging to detect,
      % mitigate and address most security issues in open source code.
    \end{wideitemize}
    ...
    % In other words, we can make sure that every line of code that we add to open source
    % is trustworthy at the moment of birth. Not just post-deployment, but pre-deployment
    % as well.
    % {\small The key to success is hitting it early, when the noise is low.}
  \end{wideitemize}
\end{frame}


% \begin{frame}[fragile]{Your code is worth millions --- why not treat it that way?}
%   \begin{wideitemize}
%     \item Need a trust system for open source that is resistant to tampering:
%     \begin{wideitemize}
%       \item Assign trust at program statement level; {\small e.g., unchecked statements,
%         statement substitutions, code re-arrangments and deletions, $\ldots$}
%       \item Trust of code depend on the reliability of author, on the reliability of
%       the developers who subsequently revised, checked, and edited the code, and on the
%       reliability of its dependencies.
%     \end{wideitemize}
%     \item The system estimates how much the different ``assertions'' in the code can be
%     trusted. {\small Estimates are computed and displayed by coloring the code's background.}
%   \end{wideitemize}
% \end{frame}

\begin{frame}[fragile]{Establishing code trust early --- when noise is low}
  ...
  \begin{columns}[T]
    \begin{column}{.7\textwidth}
      \begin{wideitemize}
        ...
        be trusted. %{\small Estimates are computed and displayed by coloring the code's background.}
      \end{wideitemize}
    \end{column}
    \hfill
    \begin{column}{.7\textwidth}
      \begin{java}
        (*@ \shade{Orange!40}{static void multmat(Matrix x, Matrix y)\{} @*)
            (*@ \shade{Orange!40}{return \shade{Red!70}{\textcolor{white}{Jama.multmat(x, y)}};} @*)
        (*@ \shade{Orange!40}{\}} @*)
      \end{java}
    \end{column}
  \end{columns}
...
% TODO(has) continuous model of establishing trust; similar to DesCert but instead of
% generating evidence, we are assigning trust to different portions of a code-base.
% possible outcomes: hot zones, failure zones, healthy areas (reusable ones), socio
% technical metrics for pinpointing fragile areas evolved by fragile teams.
